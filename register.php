  ...
  if (isset( $_POST['register'])) {
    ...
    $_SESSION['confirm_password'] = $_POST['confirm_password'];
      ...
      if (strlen($_POST['name'])<3 ) {
        ...
        header("Location:register.php?err=".urlencode("The password must be atleast 5 characters long"));
        exit();
      
      }elseif (strlen($_POST['confirm_password'])<5) {
       
        header("Location:register.php?err=".urlencode("The confirm password must be atleast 5 characters long"));
        exit();
      
      }elseif (!isUnique($_POST['email'])) {
        header("Location:register.php?err=".urlencode("E-mail is already in use. Please use another one"));
        exit();
      }
      else{
        ...
        $db->query($query);
        $message="Hi $name, Thank you for registering to our website . click on the link down below to activate your account .<br> http://localhost/Registration/activate.php?token=$token";
        mail($email, 'Account Activation', $message , 'From:innallure@gmail.com');
        header("Location:index.php?success=".urlencode("Activation E-mail sent"));
        exit();
      }
    }
?>
